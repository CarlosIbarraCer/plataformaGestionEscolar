...
class CustomAuthToken(ObtainAuthToken):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.is_active:
            ...
            # Si no hay grupo, intentamos inferir por perfil creado
            if not role_names:
                if Alumnos.objects.filter(user=user).exists():
                    role_names = ['alumno']
                elif Maestros.objects.filter(user=user).exists():
                    role_names = ['maestro']
                elif Administradores.objects.filter(user=user).exists():
                    role_names = ['administrador']

            #Si solo es un rol especifico asignamos el elemento 0
            role_names = role_names[0] if isinstance(role_names, list) else role_names
